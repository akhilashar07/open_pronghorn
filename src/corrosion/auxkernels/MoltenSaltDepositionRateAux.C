...
#include "MoltenSaltDepositionRateAux.h"

registerMooseObject("OpenPronghornApp", MoltenSaltDepositionRateAux);

InputParameters
MoltenSaltDepositionRateAux::validParams()
{
  InputParameters params = AuxKernel::validParams();
  params.addClassDescription(
      "Evaluates MoltenSaltCorrosionModel::depositionRateUmY() with a local temperature functor.");
...
MoltenSaltDepositionRateAux::MoltenSaltDepositionRateAux(const InputParameters & parameters)
...
{
  if (isNodal())
    paramError("variable", "MoltenSaltDepositionRateAux requires an elemental auxiliary variable.");
  ...
  _features.surface_class = getParam<std::string>("surface_class");
  _features.flow_factor = getParam<Real>("flow_factor");
  _features.delta_T_C = getParam<Real>("delta_T_C");
}

Real
MoltenSaltDepositionRateAux::computeValue()
{
  ...
  return _model.depositionRateUmY(_features);
}
